...
public class MRTask2 {

	public static void main(String[] args) throws Exception 
	{
		
		Configuration conf = new Configuration(); //provides access to configuration details of Mapper and Reducer
		Job job = Job.getInstance(conf, "MRJobTask2");  //Job class object is created which is given a job name "MRJobTask2", and in further lines properties are set using the "job" object
		
		job.setJarByClass(MRTask2.class);  //Main class file is included in jar
		...
		job.setMapperClass(MRMapperTask2.class); //Mapper class name is provided
		job.setReducerClass(MRReducerTask2.class); //Reducer class name is provided
